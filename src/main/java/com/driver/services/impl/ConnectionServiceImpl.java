...
public class ConnectionServiceImpl implements ConnectionService {
    ...
    public User connect(int userId, String countryName) throws Exception{
        ...
        boolean countryIsPresent = false;
        int sizeOFServiceProviderId = Integer.MAX_VALUE;
        ServiceProvider mainServiceProvider = null;
        for(ServiceProvider serviceProvider : serviceProviderList)
        {
            List<Country> countryList = serviceProvider.getCountryList();
            for(Country countries : countryList)
            {
                if(countries.getCountryName().equals(countryName.substring(0,3).toUpperCase()))
                {
                    if(sizeOFServiceProviderId > serviceProvider.getId())
                    {
                        countryIsPresent = true;
                        country = countries;
                        sizeOFServiceProviderId = serviceProvider.getId();
    ...
    public User communicate(int senderId, int receiverId) throws Exception {
        ...
        // current country, we need to connect the sender to a suitable vpn. If there are multiple options, connect using
        // the service provider having smallest id
